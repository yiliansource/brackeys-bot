...
namespace BrackeysBot.Commands
{
    public partial class ModerationModule : BrackeysBotModule
    {
        ...
        public async Task EmoteLimitAsync() 
            => await EmoteLimitAsync(Context.Channel);
        ...
        [RequireContext(ContextType.Guild)]
        public async Task EmoteLimitAsync(
            [Summary("The channel to display the limits for")] IMessageChannel channel)
        {
            if (channel == null)
            ...
            if (limits == null || !limits.ContainsKey(channel.Id) || limits.GetValueOrDefault(channel.Id).Count() == 0)
            {
                builder.WithColor(Color.Red)
                    .WithDescription($"No restrictions in {channel.Linkable()}!");
            } 
            else 
            {
                ...
                builder.WithColor(Color.Green)
                    .WithDescription($"Channel {channel.Linkable()} limits to {string.Join(", ", limitedTo)}");
            }
            ...
            [Summary("The emote to allow"), OverrideTypeReader(typeof(EmoteTypeReader))] IEmote emote)
        {
            ...
            await new EmbedBuilder().WithColor(Color.Green)
                .WithDescription($"Channel {channel.Linkable()} now limits to {string.Join(", ", newLimits.ConvertAll(ConvertToVisual))}")
                .Build()
                .SendToChannel(Context.Channel);
            ...
            [Summary("The emote to allow"), OverrideTypeReader(typeof(EmoteTypeReader))] IEmote emote)
        {
            ...
            if (limits == null) 
                throw new ArgumentException($"Channel {channel.Linkable()} has no emote restrictions");

            List<string> newLimits = limits.GetValueOrDefault(channel.Id, new List<string>());

            if (!newLimits.Remove(emote.StringVal()))
                throw new ArgumentException($"Channel {channel.Linkable()} does not allow emote {emote.Emotify()}");
